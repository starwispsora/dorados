...
void MoveTurtleBot::publish_turtlesim_msg()
{
    ...
    twist.linear.x = 0.1; // turtlebot3 limit 2m/s
...
{
    _odom_msg = *msg;
    RCLCPP_INFO(get_logger(), "Position(x : %f, y : %f, z : %f)", _odom_msg.pose.pose.position.x, 
        _odom_msg.pose.pose.position.y, _odom_msg.pose.pose.position.z);
}
